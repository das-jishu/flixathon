...
if(empty($_POST["password"])){
    ...
    echo "<script>$('#reset-password').html('Please enter password.');</script>";
    ...
    echo "<script>$('#reset-password').html('Password should be atleast 6 characters long and include one caps and one number.');</script>";
    echo "<script>$('#password').addClass('is-invalid');</script>"; 
}else{
    echo "<script>$('#reset-password').html('');</script>";
    echo "<script>$('#password').removeClass('is-invalid');</script>";
    $password = filter_var($_POST["password"], FILTER_SANITIZE_STRING); 
    if(empty($_POST["password2"])){
        $errors .= $missingPassword2;
        
        echo "<script>$('#reset-password2').html('Please confirm password.');</script>";
        echo "<script>$('#password2').addClass('is-invalid');</script>";
    }else{
        $password2 = filter_var($_POST["password2"], FILTER_SANITIZE_STRING);
        if($password !== $password2){
            $errors .= $differentPassword;
            echo "<script>$('#reset-password2').html('Passwords do not match.');</script>";
            echo "<script>$('#password2').addClass('is-invalid');</script>";
            
        }
        else {        
            echo "<script>$('#reset-password2').html('');</script>";
...
$sql = "UPDATE users SET password='$password' WHERE u_id='$user_id'";
$result = mysqli_query($link, $sql);
if(!$result){
...
if(!$result){
    ...
    echo '<div class="alert alert-success">Your password has been updated successfully!  <a class="btn btn-warning" href="./loginpage.php">LOGIN</a></div>';  
}
?>